...
class PDA(Generic[ET]):  # Probabilistic Data Association
    ...
    def gate(
        ...
        *,
        ...
        for i in range(M):
            gated.append(self.state_filter.gate(Z[i,:], filter_state, g_squared, sensor_state=sensor_state))
    ...
    def loglikelihood_ratios(
        ...
        *,
        ...
        for i in range(Z.shape[0]):
            ll[i+1] = log_PD + self.state_filter.loglikelihood(Z[i,:], filter_state)
    ...
    def association_probabilities(
        ...
        *,
        ...
        # probabilities
        # Returns beta
        return np.exp(lls - logsumexp(lls)) # TODO
    ...
    def conditional_update(
        ...
        *,
        ...
        for x in range(Z.shape[0]):
            updates.append(self.state_filter.update(Z[x,:], filter_state)) #should we go from measurment 1 and not 0?
    ...
    def step(
        ...
        *,
        ...
        filter_state_predicted = self.predict(Z, filter_state, Ts)  # TODO
        filter_state_updated = self.update(Z, filter_state, sensor_state=sensor_state)  # TODO
