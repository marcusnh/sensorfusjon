...
def logsumexp_own(a, axis=None):
    a_max = np.max(a)
    lse = a_max + np.log(np.sum(np.exp(a-a_max), axis=axis))
    return lse
...
class IMM(Generic[MT]):
    ...
    def mix_states(
        ...
        means = np.array([state.mean for state in immstate.components])
        cov = np.array([state.cov for state in immstate.components])
        mixed_states = list()
        for probs in mix_probabilities:
            mean_bar,cov_bar = gaussian_mixture_moments(probs,means,cov)
            mixed_states.append(GaussParams(mean_bar,cov_bar)) # TODO
        return mixed_states
    ...
    def mode_matched_prediction(
        ...
        #for i, filter in enumerate(self.filters):
            #modestates_pred.append(filter.predict(mode_states[i], Ts))
        # modestates_pred =self.filter.predicted(mode_states) # TODO
    ...
    def mode_matched_update(
        ...
        loglikelihoods, gauss_params = list(), list()
        gauss_params = [filter.update(z, state) for filter, state in zip(self.filters, immstate.components)]
        loglikelihoods = [filter.loglikelihood(z, state) for filter, state in zip(self.filters, immstate.components)]

        return np.array(loglikelihoods), gauss_params
    ...
    def update_mode_probabilities(
        ...
        '''
        mode_loglikelihood = z  # TODO
        numerator = mode_loglikelihood.T + np.log(immstate.weights)
        logjoint = logsumexp_own(mode_loglikelihood.T + np.log(immstate.weights))
        updated_mode_probabilities = np.exp(numerator.T - logjoint.T) 
        '''
        loglikelihood = self.loglikelihood(z,immstate)
    ...
    def update(
        ...
        _, updated_states = self.mode_matched_update(z, immstate,sensor_state=sensor_state)
    ...
    def step(
        ...
        updated_immstate = self.update(z, predicted_immstate, sensor_state)# TODO
    ...
    def loglikelihood(
        ...
        *,
        ...
        ll = np.array( [
                ...
                for fs, cs in zip(self.filters, immstate.components) ])# TODO
         
        weighted = np.log(immstate.weights)+ll
        return logsumexp(np.exp(weighted))
    ...
    def reduce_mixture(
        ...
        # try ton transpose the component matrix
        # mode:prob list p(sk)
        # mode_condition blir p(ak[sk)])
        # Hint list_a of lists_b to list_b of lists_a: zip(*immstate_mixture.components)
        components_per_mode = zip(*[comp.components for comp in immstate_mixture.components])
        for fs, a, mode_conditioned in zip(self.filters, components_per_mode, mode_conditioned_component_prob):
            states = fs.reduce_mixture(MixtureParameters(mode_conditioned,a))
            mode_states.append(states)  # TODO:
    ...
    def estimate(self, immstate: MixtureParameters[MT]) -> GaussParams:
        ...
        # means = np.array([state.mean for state in immstate.components])
        # covs = np.array([state.cov for state in immstate.components])
        # estimate = gaussian_mixture_moments(immstate.weights, means, covs)
        # return estimate
    ...
    def gate(
        ...
        #check for all modes:
        gated_per_mode = []
        NIS, NISes = self.NISes(z,immstate)
        for i in NISes:
            gated_per_mode.append(i <= gate_size) #TODO

        gated =np.any(gated_per_mode)# TODO
