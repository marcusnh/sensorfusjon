...
x_bar_init = [4,1,3,1]
P_bar_init = np.diag([500,500,100,100])**2# TODO: ArrayLike with 4 x 4 elements, hint: np.diag
...
RMSE_pred = np.sqrt(stats['dists_pred']**2)  # TODO
RMSE_upd = np.sqrt(stats['dists_upd']**2)  # TODO same for 'dists_upd'

fig3, ax3 = plt.subplots(num=3, clear=True)

ax3.plot(*Xgt.T[:2])
ax3.plot(*ekfupd_list.mean.T[:2])
#RMSEs_str = ", ".join(map("{:.2f}".format(*RMSE_pred, *RMSE_upd)))
for v in (*RMSE_pred, *RMSE_upd):
    RMSEs_str=", {}".format(v)
#RMSEs_str = "fillout"
ax3.set_title(
...
RMSE_pred = np.sqrt(np.mean(stats_array['dists_pred']**2, axis=2))  # TODO
RMSE_upd = np.sqrt(np.mean(stats_array['dists_upd']**2,axis=2))  # TODO
...
CINIS = np.array(scipy.stats.chi2.interval(confprob, df=2*K))/K  # TODO confidence intervall for NIS, hint: scipy.stats.chi2.interval
print("CINIS: ", CINIS)

# plot
fig4 = plt.figure(4, clear=True)
ax4 = plt.gca(projection='3d')
print(sigma_a_list, sigma_z_list)
...
confprob = 0.9  # TODO
CINEES =  np.array(scipy.stats.chi2.interval(confprob, df=4*K))/K # TODO, not NIS now, but very similar
print("CINEES:",CINEES)
plt.savefig(pp, format='pdf')
