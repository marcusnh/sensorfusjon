...
# set styles
try:
    # installed with "pip install SciencePLo
...
if use_pregen:
    ...
    P0 = np.diag([50, 50, 10, 10, np.pi / 4]) ** 2  # Stod pi og ikke np.pi her
    # model parameters
    sigma_a_true = 0.25
...
tracker_update = ekf.GaussParams(x_bar_init, P_bar_init)  # Er dette riktig? Hvem vet. Stod init_state før.
tracker_update_list = []
...
for k, (Zk, x_true_k) in enumerate(zip(Z, Xgt)):
    ...
    NEES[k] = tracker.state_filter.NEES(tracker_update, x_true_k[:-1])  # TODO
    x_diff = tracker_update.mean - x_true_k[:-1]
    NEESpos[k] = (x_diff[:2]).T @ np.linalg.inv(tracker_update.cov[:2, :2]) @ (x_diff[:2])  # TODO
    NEESvel[k] = (x_diff[2:4]).T @ np.linalg.inv(tracker_update.cov[2:4, 2:4]) @ (x_diff[2:4])  # TODO
