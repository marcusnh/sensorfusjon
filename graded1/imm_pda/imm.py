...
class IMM(Generic[MT]):
    ...
    def loglikelihood(
        ...
        *,
        ...
        ll = logsumexp(np.exp(weighted))  # Skal det være exp(weighted)?
    ...
    def reduce_mixture(
        ...
        components = zip(*[comp.components for comp in immstate_mixture.components])
        
        mode_states: List[GaussParams] = [
            fs.reduce_mixture(MixtureParameters(mccp, c)) for fs, mccp, c in zip(
    ...
    def gate(
        ...
        NISes = self.NISes(z, immstate, sensor_state=sensor_state)[1]

        gated_per_mode = []
        for i in NISes:
            gated_per_mode.append(i <= gate_size_square)  # TODO
